...
nm_utils_ip4_netmask_to_prefix(guint32 netmask)
{
    ...
    return ((netmask != 0) ? (32 - __builtin_ctz(ntohl(netmask))) : 0);
}
...
#define hwaddr_aton(asc, buffer, buffer_len, out_len) \
    nm_utils_hexstr2bin_full((asc), FALSE, TRUE, FALSE, ":-", 0, (buffer), (buffer_len), (out_len))
...
const char *
nm_utils_hwaddr_ntoa_buf(gconstpointer addr,
                         gsize         addr_len,
                         gboolean      upper_case,
                         char *        buf,
                         gsize         buf_len)
