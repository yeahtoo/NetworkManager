...
nmc_string_screen_width(const char *start, const char *end)
{
    ...
    while (p < end) {
        width += g_unichar_iswide(g_utf8_get_char(p))
                     ? 2
                     : g_unichar_iszerowidth(g_utf8_get_char(p)) ? 0 : 1;
                   ...
                   char **               out_to_free)
{
    ...
    if (field_name)
        ...
        value = field->value
                    ? (is_array ? (free_value = g_strjoinv(" | ", (char **) field->value))
                                : (*((const char *) field->value)) ? field->value : not_set_str)
                    : not_set_str;
    }
