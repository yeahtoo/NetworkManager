...
#define ACD_TRACK_FMT \
    "[" NM_HASH_OBFUSCATE_PTR_FMT "," NM_HASH_OBFUSCATE_PTR_FMT "," NM_HASH_OBFUSCATE_PTR_FMT "]"
                          ...
                          const NML3ConfigNotifyPayload *pay_load)
{
    ...
    g_signal_emit(self, signals[SIGNAL_NOTIFY], 0, (int) notify_type, pay_load);
}
...
static void
_l3cfg_externally_removed_objs_drop(NML3Cfg *self, int addr_family)
{
    ...
    _LOGD("externally-removed: untrack IPv%c", nm_utils_addr_family_to_char(addr_family));
...
_l3cfg_externally_removed_objs_drop_unused(NML3Cfg *self)
{
    ...
    while (g_hash_table_iter_next(&h_iter, (gpointer *) &obj, NULL)) {
        if (!nm_l3_config_data_lookup_route_obj(self->priv.p->combined_l3cd, obj)) {
...
_l3_acd_platform_commit_acd_update(NML3Cfg *self)
{
    ...
    nm_l3cfg_platform_commit(self, NM_L3_CFG_COMMIT_TYPE_AUTO, AF_INET, NULL);
}
...
static void
_l3_acd_data_notify_acd_failed(NML3Cfg *self, AcdData *acd_data, gboolean force_all)
{
...
handle_probing_acd_good:
    switch (acd_data->acd_state) {
    case ACD_STATE_INIT:
        _LOGT_acd(acd_data, "state: probe-done good (%s, inializingbcwin)", log_reason);
        acd_data->acd_state    = ACD_STATE_PROBE_DONE;
        acd_data->probe_result = TRUE;
        goto handle_probe_done;
    case ACD_STATE_PROBING:
...
nm_l3cfg_mark_config_dirty(NML3Cfg *self, gconstpointer tag, gboolean dirty)
{
    ...
    idx = 0;
    while (TRUE) {
        idx = _l3_config_datas_find_next(l3_config_datas, idx, tag, NULL);
        if (idx < 0)
            return;
                    ...
                    NML3ConfigMergeFlags  merge_flags)
{
    ...
    idx = _l3_config_datas_find_next(l3_config_datas, 0, tag, replace_same_tag ? NULL : l3cd);
                     ...
                     const NML3ConfigData *l3cd)
{
    ...
    changed = FALSE;
    while (TRUE) {
        idx = _l3_config_datas_find_next(l3_config_datas, idx, tag, l3cd);
        if (idx < 0)
            return changed;

        if (only_dirty && !_l3_config_datas_at(l3_config_datas, idx)->dirty) {
            idx++;
            continue;
        }
                              ...
                              gboolean *             out_changed_combined_l3cd)
{
    ...
    _l3_acd_data_add_all(self, l3_config_datas, l3_config_datas_len);
                 ...
                 gboolean *        out_final_failure_for_temporary_not_available)
{
    ...
    if (self->priv.p->combined_l3cd) {
        ...
        addresses = nm_dedup_multi_objs_to_ptr_array_head(
            nm_l3_config_data_lookup_objs(self->priv.p->combined_l3cd,
                                          NMP_OBJECT_TYPE_IP_ADDRESS(IS_IPv4)),
            predicate,
            self->priv.p->externally_removed_objs_hash);
        ...
        route_table_sync =
            nm_l3_config_data_get_route_table_sync(self->priv.p->combined_l3cd, addr_family);
    }
                         ...
                         gboolean *        out_final_failure_for_temporary_not_available)
{
    ...
    if (NM_IN_SET(addr_family, AF_UNSPEC, AF_INET)) {
        if (!_platform_commit(self,
                              AF_INET,
                              commit_type,
                              out_final_failure_for_temporary_not_available))
            success = FALSE;
    }
    ...
    if (acd_was_pending && !self->priv.p->acd_is_pending)
...
const NMPObject *
nm_l3cfg_get_best_default_route(NML3Cfg *self, int addr_family)
{
    nm_assert(NM_IS_L3CFG(self));
