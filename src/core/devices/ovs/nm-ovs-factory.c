                     ...
                     NMDeviceFactory *self)
{
    ...
    /* Failing an OVS interface device is almost always the right
     ...
     */
    /* FIXME: a more correct solution would be that ->enslave_slave() and
     * ->release_slave() become asynchronous and the caller wait for
     * completion before proceeding.
     */
    if (device_type == NM_DEVICE_TYPE_OVS_INTERFACE && !nm_device_get_master(device)
        && !nm_device_sys_iface_state_is_external_or_assume(device))
        return;
